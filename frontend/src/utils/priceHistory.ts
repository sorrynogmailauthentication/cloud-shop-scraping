...
/** Format `YYYY-MM-DD` for UI (e.g. "25 мар 2026"). */
export function formatYmdDisplay(ymd: string): string {
  ...
  const ruMonthsShort = ['янв', 'фев', 'мар', 'апр', 'мая', 'июн', 'июл', 'авг', 'сен', 'окт', 'ноя', 'дек'];
  return `${t.getDate()} ${ruMonthsShort[t.getMonth()]} ${t.getFullYear()}`;
}
